...
def configure_json_dict(json_dict, config_dict):
    ...
    #                                  empty dictionarys of lists defend missing & duplicate input melds
    
    keys_d = get_json_keys_config_dict(json_dict)
    #                                  short_name: full_name (to reference XX.template.json from config.txt)
    
    json_keys_counter = Counter(json_dict.keys())
    #                                  to check that all keys get some value from the config file
    ...
    for k, v in json_keys_counter.items():
        #                              Counter object starts at one ergo json key is missing if v is 1
        if v < 2:
