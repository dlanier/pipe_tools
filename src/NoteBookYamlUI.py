...
YAML_FILE_SELECT_LABEL = 'Widget run_parameters File Selector: "Select-Revert" must be clicked after making selection'
PARAMETERS_EDITOR_LABEL = 'Key - Value Parameter Selector: Click "Edit" to edit Value, Then "Set" to set Value'
SAVE_SHOW_NEW_LABEL = '"Save" - edited parameters to AA_Save_As_file_name: Name   -' + \
                      '"New Parameter" - add parameter "Show All" | "Hide" parameters'
SAVE_OUTPUT_KEY = 'AA_Save_To_directory'
SAVE_OUTPUT_VAL = 'AA_Save_As_file_name'
...
class ParameterSetWidgets():
    ...
    def __init__(self, input_data_dir=None, file_types=PARAMETER_FILE_TYPES):
        ...
        self.ed_par_button.file_save_show_label = widgets.Label(value=SAVE_SHOW_NEW_LABEL)
        self.view_save_buttons = widgets.VBox([self.ed_par_button.file_save_show_label,
                                               widgets.HBox([self.save_run_parameters_button,
    ...
    def get_selected_file_name(self):
        ...
        if self.select_file_button.file_selector.value == 'Refresh':
            ...
            self.select_file_button.file_selector.value = options[0]
        else:
    ...
    def _show_html_parameters(self):
        ...
        self.ed_par_button.parameters
        """
        self.all_parameters_view_box.value = get_run_parameters_html_table(self.get_edited_run_parameters())
    ...
    def get_edited_run_parameters(self):
        ...
        run_parameters_dict = {k: self.ed_par_button.parameters[k] for k in keys_list}
        for rp_key, rp_val in run_parameters_dict.items():
            if rp_val in TRUE_STRINGS:
                run_parameters_dict[rp_key] = True
            elif rp_val in FALSE_STRINGS:
                run_parameters_dict[rp_key] = False
            elif rp_val in NONE_STRINGS:
                run_parameters_dict[rp_key] = None

        return run_parameters_dict

    def cell_display_run_parameters(self):
        """ print formatted dict to std out """
        std_out_run_parameters_str(self.get_edited_run_parameters(), std_output=True)
    ...
    def _save_parameters(self, button):
        ...
        if SAVE_OUTPUT_KEY in run_parameters:
            directory_name  = run_parameters[SAVE_OUTPUT_KEY]
        else:
            directory_name = self._input_file_name

        if SAVE_OUTPUT_VAL in run_parameters:
            file_name   = run_parameters[SAVE_OUTPUT_VAL]
        else:
    ...
    def _open_run_parameters(self, run_directory, run_file):
        ...
        run_parameters[SAVE_OUTPUT_VAL] = file_name
        if self._input_dir_name is None or os.path.isdir(self._input_dir_name) == False:
            run_parameters[SAVE_OUTPUT_KEY] = run_directory
        else:
            run_parameters[SAVE_OUTPUT_KEY] = self._input_dir_name
