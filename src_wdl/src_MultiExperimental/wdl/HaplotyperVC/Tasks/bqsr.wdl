...
task bqsrTask {
   ...
   Boolean CaptureRecalibratedbam        # Variable to capture the recalibrated BAM

   command <<<
       source ${BashPreamble}
       /bin/bash ${BqsrScript} -s ${SampleName} -S ${Sentieon} -G ${Ref} -t ${SentieonThreads} -b ${InputBams} -k ${BqsrKnownSites} -e ${BqsrEnvProfile} -F ${BashSharedFunctions} ${DebugMode} ${CaptureRecalibratedbam}
   >>>
   ...
   output {
      ...
      File RecalibratedBam = "${SampleName}.recalibrated.bam"
   }
}
