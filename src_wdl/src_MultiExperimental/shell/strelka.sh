...
#############################################################################
 ...
 strelka.sh        
		   ...
		   -F		<shared_functions>
	           -i           <indel_GT_fix_python_script>
                   -p           <snp_GT_fix_python_script>
		   -m		<Manta optional indel candidates>
...
EXAMPLES:
strelka.sh -h
strelka.sh -N normal.bam -T tumor.bam -g reference_genome.fasta -I /path/to/strelka/install -B /path/to/BCFTools -S /path/to/samtools -Z /path/to/bgzip -t <number_of_threads> -e /path/to/envprofile.file -F /path/to/MayomicsVC/shared_functions.sh -i /path/to/fix_indels.py -p /path/to/fix_snps.py -m <--indelCandidates ./manta/results/variants/variant_type.vcf.gz> -o "'--extra_option'" -O "'extra_bcf_options'"
...
while getopts ":hs:N:T:g:I:B:S:Z:t:e:F:i:p:m:o:O:d" OPT
do
        case ${OPT} in
		...
		F )  # Shared functions 
                        ...
                        checkArg
                        ;;
		i )  # Path to fixStrelka_GT_indels.py
			FIX_INDEL_GT=${OPTARG}
			checkArg
			;;
		p )  # Path to fixStrelka_GT_snvs.py
			FIX_SNV_GT=${OPTARG}
			checkArg
			;;
		m )  # Manta optional indelCandidates
			MANTA=${OPTARG}
			checkArg
			;;
...
#checkVar "${MANTA_ANALYSIS_PATH+x}" "Missing manta analysis directory option: -A" $LINENO
...
checkVar "${FIX_INDEL_GT+x}" "Missing Fix GT Indel python script option: -i" $LINENO
checkFile ${FIX_INDEL_GT} "Fix GT INDEL python script ${FIX_INDEL_GT} is empty or does not exist." $LINENO

checkVar "${FIX_SNV_GT+x}" "Missing Fix GT SNV python script option: -p" $LINENO
checkFile ${FIX_SNV_GT} "Fix GT SNV python script ${FIX_SNV_GT} is empty or does not exist." $LINENO

checkVar "${MANTA}" "Missing Manta usage option option: -m"
...
MANTA_OPTION_PARSED=`sed -e "s/'//g" <<< ${MANTA}`
...
${INSTALL}/bin/configureStrelkaSomaticWorkflow.py \
    ...
    --runDir=./strelka \
    $MANTA_OPTION_PARSED >> ${TOOL_LOG} 2>&1
...
zcat ./strelka/results/variants/somatic.indels.vcf.gz | python ${FIX_INDEL_GT} > somatic.indels.fixed.vcf 2>> ${TOOL_LOG} 
...
zcat ./strelka/results/variants/somatic.snvs.vcf.gz | python ${FIX_SNV_GT} > somatic.snvs.fixed.vcf 2>> ${TOOL_LOG} 
...
${BCF}/bcftools merge ${BCFTOOLS_OPTIONS_PARSED} --force-samples ./*fixed.vcf.bgz > ${SAMPLE}.vcf 2>> ${TOOL_LOG}
EXITCODE=$?
trap - INT TERM EXIT
checkExitcode ${EXITCODE} $LINENO
